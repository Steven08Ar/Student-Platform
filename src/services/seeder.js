...
import { oopFundamentalsCourseData } from "@/data/oopFundamentalsCourseData";
...
export const seedOOPCourse = async (userId) => {
    try {
        console.log("Starting OOP Course Seeding...");

        // 1. Create Class
        const classDoc = await createClass({
            title: oopFundamentalsCourseData.title,
            description: oopFundamentalsCourseData.description,
            color: oopFundamentalsCourseData.color
        }, userId);
        const classId = classDoc.id;
        console.log(`OOP Class Created: ${classId}`);

        // 2. Iterate Modules
        for (const moduleData of oopFundamentalsCourseData.modules) {
            const modDoc = await createModule(classId, moduleData.title, moduleData.order);
            const moduleId = modDoc.id;
        ...
        console.log("OOP Course Seeding Complete!");
        return true;
    } catch (error) {
        console.error("OOP Seeding Failed:", error);
        return false;
    }
};
